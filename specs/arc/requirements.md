...
- As a package client, I want typed ARC-3 and ARC-69 metadata that validates and round-trips through JSON.
- As a collection publisher, I want ARC-19 template and reserve-address helpers that resolve deterministic asset URLs.
- As an IPFS client, I want CID and URL values that parse, render, resolve, and serialize without network access.
- As a maintainer, I want validation failures and warnings preserved separately and mapped to meaningful native tests.

## Constraints

- The package remains Swift 6 and follows the platform declarations in `Package.swift`.
- CID and ARC-19 reserve encoding currently support SHA2-256 hashes of exactly 32 bytes.
- ARC-19 template parsing accepts only CID versions 0 and 1 and the declared codec set.
- Metadata values remain synchronous, deterministic, Codable, and `Sendable`; the module performs no I/O.
- This migration does not modify `Sources/`, `Tests/`, product behavior, or public API.

## Out of Scope

- Fetching or pinning IPFS content, choosing gateways dynamically, or verifying remote content integrity.
- Creating, signing, or submitting Algorand assets and transactions.
- Adding missing collection, array, Boolean, or null variants to `PropertyValue`.
- Repairing the pre-existing no-assertion CID test or changing existing implementation behavior.
